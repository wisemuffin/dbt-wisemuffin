...
with campaigns as (
    ...
    select 
        {{dbt_utils.surrogate_key(['app_id'])}} as app_sk,
        {{dbt_utils.surrogate_key(['campaigns.email_campaign_id'])}} as email_campaign_sk,
        {{dbt_utils.surrogate_key(['content_id'])}} as content_sk,
        campaigns.*,
        {% for metric in var('email_metrics') %}
